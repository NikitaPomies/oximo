...
fn dense_indexed_param_used_in_model() {
    ...
    assert!((m.param_value_idx(&c, 2usize).unwrap().unwrap() - 30.0).abs() < f64::EPSILON);
    m.set_param_idx(&c, 2usize, 5.0).unwrap();
    assert!((m.param_value_idx(&c, 2usize).unwrap().unwrap() - 5.0).abs() < f64::EPSILON);
}
...
fn multi_index_param_is_dense() {
    ...
    assert!(
        (m.param_value_idx(&weight, (1usize, 1usize)).unwrap().unwrap() - 4.0).abs() < f64::EPSILON
    );
}
...
fn string_keyed_indexed_param_is_sparse() {
    ...
    assert!((m.param_value_idx(&cost, "b").unwrap().unwrap() - 2.0).abs() < f64::EPSILON);
    assert!(m.param_value_idx(&cost, "z").unwrap().is_none());
}
...
fn constant_indexed_param_does_not_reference_index() {
    ...
    assert_eq!(k.len(), 3);
    assert!((m.param_value_idx(&k, 0usize).unwrap().unwrap() - 7.0).abs() < f64::EPSILON);
}
...
fn filtered_indexed_param_is_sparse() {
    ...
    assert!(m.param_value_idx(&even, 1usize).unwrap().is_none());
    assert!((m.param_value_idx(&even, 4usize).unwrap().unwrap() - 4.0).abs() < f64::EPSILON);
}
